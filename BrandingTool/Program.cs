...
namespace BrandingTool
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            foreach (var site in branding.Descendants("site"))
            {
                ...
                using (ClientContext clientContext = new ClientContext(siteUrl))
                {
                    ...
                    Console.WriteLine("{1}Deploying Branding to {0}", siteUrl, Environment.NewLine);
                    clientContext.Load(clientContext.Web.RootFolder);
                    clientContext.ExecuteQuery();

                    foreach (XElement element in site.Descendants())
                    {
        ...
        private static void UploadFile(ClientContext clientContext, XElement element)
        {
            ...
            Console.WriteLine("{2} - Uploading {0} to {1}", Path.GetFileName(filePath), String.Concat(libraryName, "/", folder).TrimEnd(trimChars), Environment.NewLine);
            var destFolder = library.RootFolder;
            if (folder.Length > 0)
            {
                destFolder = clientContext.Web.EnsureFolder(library.RootFolder, folder);
            }
            destFolder.UploadFile(filePath);
        }
